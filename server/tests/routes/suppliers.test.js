...
jest.mock('../../middleware/roleCheck', () => ({
  ...
  procurementAccess: jest.fn((req, res, next) => next())
}));
...
describe('Supplier Routes', () => {
  describe('GET /api/suppliers', () => {
    ...
    test('should handle errors', async () => {
      ...
      expect(response.body.message).toBe('Error fetching suppliers');
    });
  });
  ...
  describe('GET /api/suppliers/:id', () => {
    test('should return a single supplier', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.data.name).toBe(mockSupplier.name);
      expect(auth).toHaveBeenCalled();
      expect(staffAccess).toHaveBeenCalled();
    });
  ...
  describe('POST /api/suppliers', () => {
    test('should create a new supplier', async () => {
      ...
      expect(response.body.data.name).toBe(mockSupplier.name);
      expect(auth).toHaveBeenCalled();
      expect(procurementAccess).toHaveBeenCalled();

      // Verify supplier was saved to database
      const supplierInDb = await Supplier.findById(response.body.data._id);
