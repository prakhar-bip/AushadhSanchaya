...
describe('Products API', () => {
  ...
  describe('PUT /api/products/:id', () => {
    test('should update a product', async () => {
      ...
      expect(res.body.data.unitPrice).toBe(updatedInfo.unitPrice);
      
      // Verify fields not included in update remain unchanged
      expect(res.body.data.genericName).toBe(sampleProduct.genericName);
    });
  ...
  describe('PATCH /api/products/:id/stock', () => {
    test('should increase stock quantity', async () => {
      const adjustment = { adjustment: 50, reason: 'New shipment received' };
      const res = await request(app)
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data.stockQuantity).toBe(sampleProduct.stockQuantity + adjustment.adjustment);
    });

    test('should decrease stock quantity', async () => {
      const adjustment = { adjustment: -30, reason: 'Sales out' };
      const res = await request(app)
        .patch(`/api/products/${productId}/stock`)
        .set('Authorization', `Bearer ${token}`)
        .send(adjustment);
    ...
    test('should return 404 if product not found', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${token}`)
        .send({ adjustment: 10, reason: 'Inventory check adjustment' });
